...
{
  //String output = String::format("%d,%s,%s,%s,%s", payloadType, deviceID.c_str(), LocationService::instance().getGPStime().c_str(), LocationService::instance().getGPSdata().c_str(), data.c_str());
  ...
  if (client.connect(TCP_ENDPOINT, 1024) | TCP_GHOSTWRITE)
  {
    ...
    for (i = 0; i < files_to_dump; i++)
    {
      ...
      if (file.size() > 0)
      {
        ...
        if (TCP_GHOSTWRITE)
        {
          Serial.println("Sending the following data over TCP");
          Serial.println(body);
        }
        if (!TCP_GHOSTWRITE)
        {
          ...
          Serial.print("TCP body write result: ");
          Serial.println(body_resp);
          client.flush();
        }
        free(body);
      }
